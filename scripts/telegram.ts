...
class TelegramAPIResponse {
    ...
    private responseObject: { [key: string]: string };
    /**
     ...
     */
    constructor(statusCode: number, responseJSON: string) {
        ...
        Object.keys(temp).forEach( (k:string) => {
            this.responseObject[k] = JSON.stringify(temp[k as keyof typeof temp]);
        });
    }
}
 ...
 * @returns {string}
 */
function humanFileSize(sz: BigInt) {
 ...
 * @returns {string}
 */
function makeMessage(notice: zeektypes.Notice_Info) {
 ...
 */
function sendTelegramNotice(notice: zeektypes.Notice_Info) {
