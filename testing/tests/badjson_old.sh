...
# @TEST-REQUIRES: WANT_EQ_OR_OLDER=18 zeek ${SCRIPTS}/node_version_test.js
# @TEST-EXEC: btest-bg-run api "MOCK_PORT=$(echo ${ZEEK_MOCK_PORT} | sed 's|/tcp||') zeek ${SCRIPTS}/mock_server.js > server.out"
# @TEST-EXEC: MOCK_PORT=$(echo ${ZEEK_MOCK_PORT} | sed 's|/tcp||') NODE_EXTRA_CA_CERTS=${ETC}/ca.pem TELEGRAM_ENDPOINT="https://127.0.0.1:${MOCK_PORT}/corrupt" zeek $PACKAGE ${SCRIPTS}/mock_client.zeek > client.out
